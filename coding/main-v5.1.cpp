...
/*                                                                           */
/*                                                                           */
/*     dev. by Carlo Giocoli - cgiocoli@gmail.com (99.99% of the hard work)  */
/*****************************************************************************/
...
double getY(std:: vector<double> x, std:: vector<double> y,double xi){  // Interpolated routine to calculate y(xi)
  int nn = x.size();
  if(x[0]<x[nn-1]){         
...
void readParameters(int *npix, double *boxl,
		    ...
		    string *partinplanes, int *noSNAP, double *bufferdeg, string *directory,string *suffix){ 

  string butstr;
  
  cin >> butstr; // number of pixels
  cin >> *npix;
  cin >> butstr; // boxl
  cin >> *boxl;
  cin >> butstr; // source redshift
  cin >> *zs;
  cin >> butstr; // field of view in degrees
  cin >> *fov;
  cin >> butstr; // file with the redshift list it may contain three columns: snap 1/(1+z) z
  cin >> *filredshiftlist;
  cin >> butstr; // path where the snaphosts are located
  cin >> *pathsnap;
  cin >> butstr; // simulation name (prefix infront at the snap file)
  cin >> *simulation;
  cin >> butstr;  // number of files per snapshot
  cin >> *nfiles;
  cin >> butstr; // path and file name of the comoving distance file (if not available you may use CosmoLib)
  cin >> *idc;
  cin >> butstr; // path of SUBFIND if NO I will not look for them
  cin >> *subfiles;
  cin >> butstr; // seed for the random location of the center
  cin >> *seedcenter;
  cin >> butstr; // seed for the random selection of the dice face
  cin >> *seedface;
  cin >> butstr; // seed for the selection of the sign of the coordinates
  cin >> *seedsign;  
  cin >> butstr; // which particles in the planes (ALL: one file for all, or NO: one for each)
  cin >> *partinplanes;  
  cin >> butstr; // if 0 read also snaphosts if =/0 only subs and fof area read
  cin >> *noSNAP;  
  cin >> butstr; // size of the buffer region in degrees for subs and fof
  cin >> *bufferdeg;
  cin >> butstr;//directory to save FITS files  
  cin >> *directory;
  cin >> butstr;//Sufix to save FITS files  
  cin >> *suffix;
}; 
...
int main(){ 
  ...
  readParameters(&npix,&boxl,&zs,&fov,
		 ...
		 &partinplanes,
		 &noSNAP,&bufferdeg,&directory,&suffix);  // Reading the parameters File
...
//cout << filredshiftlist;
  ...
  for(int i=0;i<blD.size();i++){
    switch(i){
      case 0:{
        if(blD[i]>boxl){// Duplicate the first plane in case it begins at dl bigger than Box_size
  ...
  if(blD[0]!=0){
    nsnaps++;
    blD.insert(blD.begin(),0);
    blD2.insert(blD2.begin(),blD[1]);

    double dlbut = (blD[0] + blD2[0])*0.5; // half distance between the two!
    bzsimlens.insert(bzsimlens.begin(),getY(dl,zl,dlbut));
    breplication.insert(breplication.begin(),0);
    bfromsnap.insert(bfromsnap.begin(),bfromsnap[0]);
    blsnap.insert(blsnap.begin(),blsnap[0]);
    blred.insert(blred.begin(),blred[0]);
  }
  ...
  for(int nsnap=0;nsnap<nsnaps;nsnap++){
    ...
    float rcase = blD[nsnap]/boxl; //New code
    int rcase_index = breplication[nsnap]; //New code
	...
	for (int pp=0; pp<data.npart[0]; pp++){
	  ...
	  xb = sgnX[rcase_index]*(((num_float1)/data.boxsize));
	  yb = sgnY[rcase_index]*(((num_float2)/data.boxsize));
	  zb = sgnZ[rcase_index]*(((num_float3)/data.boxsize));
	...
	// vector<double> ra(0),dec(0);
        ...
        cout << "Min distance: "<< blD[nsnap]/data.boxsize*1.e+3<< " "<<blD2[nsnap]/data.boxsize*1.e+3 << endl;
...
// cout << " n0: totPartxy0 " << totPartxy0 << endl;
	...
	if(totPartxy0>0){
        	mapxy0 = gridist_w(xs,ys,ms,npix);
        }

	// re-normalize to the total mass!
	double mtot0=0.;
        double mnorm=0;
        for(int i=0;i<ms.size();i++)if(ms[i]<1) mnorm+=ms[i];
