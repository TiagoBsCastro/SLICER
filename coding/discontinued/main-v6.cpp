...
float weight (float ixx, float ixh, double dx) { //TSC weight
  float DD = ixx-ixh;
  float x=fabs(DD)/dx;
  float w;    
...
double WC6_kernel (double z, void * params){
  ...
  if(r<=1)
    w=1365./(64*M_PI)*( pow(1 - r,8)*(1 + 8*r + 25*pow(r,2) + 32*pow(r,3)) );
  else w=0.;

  return w;
}

double WC6_projected_kernel (double x, double y, double h){

  double params[] = {x/h,y/h};
  double result, error;
  gsl_integration_workspace * gsl_integrate = gsl_integration_workspace_alloc (100);

  gsl_function F;
  F.function = &WC6_kernel;
  F.params = &params;

  gsl_integration_qags (&F, -1, 1, 0, 1e-6, 100, gsl_integrate, &result, &error);

  gsl_integration_workspace_free(gsl_integrate);
...
// grid points distribution function with != wheights using hydro part. SPH-TSC kernel convolution
valarray<float> gridist_w (vector<float> x, vector<float> y , vector<float> w, vector<float> h, int nn){
  ...
  for (int i=0; i<n0; i++){
    ...
    double rx=x[i]/dl;
    int grx=int(rx); // x pos on the grid {0..nn}
    double ry=y[i]/dl;
    int gry=int(ry); // y pos on the grid {0..nn}
    double r=h[i]/dl;
    int gr=int(r)+1; // size of the SPH radius in bin's size

    for (int ix=grx-gr; ix<=grx+gr; ix++){

      for (int jy=gry-gr; jy<=gry+gr; jy++){

        if(ix<nn && jy<nn && ix>0 && jy>0)
          
          grxy[ix+nn*jy] += w[i]*WC6_projected_kernel(ix-rx,jy-ry,r);
...
//cout << filredshiftlist;
  ...
  for(int nsnap=0;nsnap<nsnaps;nsnap++){
	...
	if(totPartxy0>0){
        	mapxy0 = gridist_w(xs,ys,ms,hs,npix);
        }
