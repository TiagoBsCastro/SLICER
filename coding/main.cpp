...
void readParameters(int *npix, double *boxl,
  ...
  cin >> *filredshiftlist;
  cin >> butstr; // file with the snapshot list available 
  cin >> *filsnaplist;
...
//cout << filredshiftlist;
  ...
  if(redlist.is_open()){
	...
	exit(1);
      ...
      snapToRedshift[buta] = butc; // Assigning z=butc to array pos buta!!
    }
  }else{
  ...
  ifstream snaplist;
  snaplist.open(filsnaplist.c_str());  // Opening snaplist.txt
  vector <int> lsnap;
  vector<double> lred;
  vector<double> lD;
  if(snaplist.is_open()){
    ...
    double zn;
    int check=0;
    while(snaplist >> s){ // Looping over the snaplists s in [136,128,...]
      zn = getY(dtsnaplist,tredlist,double(s));  // getY???
      if(zn>zs) check=1;
      if(check==0){
	lsnap.push_back(s);
	lred.push_back(zn);
  ...
  for(int nsnap=0;nsnap<nsnaps;nsnap++){
	...
	if(hydro){
		...
		fin >> block;
        	...
        	cout << endl << "If masses were read correctly next box should be U   " << endl;
	        cout << "Name:                                            ";
	        cout << block.name[0];
	        cout << block.name[1];
	        cout << block.name[2];
