...
int main(int argc, char **argv)
{
  ...
  for (auto i = 0u; i < lens.ld.size(); i++)
  {
  ...
  for (int isnap = 0; isnap < lens.nplanes; isnap++)
  {
    ...
    unsigned int intdiv, remaindiv, ffmin, ffmax;
    ...
    else
    {
      ...
      for (unsigned int ff = ffmin; ff < ffmax; ff++)
      {
        ...
        xx[0][1] = &halos->yy0[0];
        xx[0][2] = &halos->zz0[0];
        xx[1][0] = &subhalos->xx0[0];
        xx[1][1] = &subhalos->yy0[0];
        xx[1][2] = &subhalos->zz0[0];
        vv[0][0] = &subhalos->vx0[0];
        vv[0][1] = &subhalos->vy0[0];
        vv[0][2] = &subhalos->vz0[0];
        for (int i = 1; i < 6; i++)
        {
          for (int j = 0; j < 3; j++)
          {
  ...
  gsl_spline_free(getDl);
  gsl_spline_free(getZl);
  gsl_interp_accel_free(accGetDl);
  gsl_interp_accel_free(accGetZl);
