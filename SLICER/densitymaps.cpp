            ...
            gsl_interp_accel *accGetDl, double dlens)
{
  ...
  if (zsnap.empty())
  {
    return -1; // Indicates that no index could be valid
  }

  unsigned int pos = 0;
  double aux = 99999;

  for (size_t i = 0; i < zsnap.size(); i++)
  {
                ...
                int numOfLensPerSnap, int myid)
{
  ...
  do
  {
    ...
    for (size_t i = pos_temp; i < nsnaps; i++)
    {
      ...
      if (itest == -1)
      {
        ...
        cerr << "snapred is an empty array!" << endl;
        cerr << "Check your snapshot list file." << endl;
        return 1;
      }
      if ((size_t)itest >= snapred.size())
      {
  ...
  for (int i = 0; i < nrepi + 1; i++)
    lens.replication.push_back(nrep); // Last plane replications
  if (myid == 0)
  {
  ...
  for (size_t i = 0; i < lens.fromsnap.size(); i++)
  {

    if (myid == 0)
    {
      cout << lens.zsimlens[i] << " planes = " << lens.ld[i] << "  " << lens.ld2[i] << "  " << lens.replication[i] << " from snap " << lens.fromsnap[i] << endl;
      planelist << i << "   " << lens.zsimlens[i] << "   " << lens.ld[i] << "   " << lens.ld2[i] << "   " << lens.replication[i] << "   " << lens.fromsnap[i] << "   " << lens.zfromsnap[i] << "  "
                  ...
                  int numOfLensPerSnap, int myid)
{
  ...
  random.x0.resize(nrandom);
  random.y0.resize(nrandom);
  random.z0.resize(nrandom);
  random.sgnX.resize(nrandom);
  random.sgnY.resize(nrandom);
  random.sgnZ.resize(nrandom);
...
/*If FixedPLCVertex directive is activated the snapshots
  are always centered on x,y,z = 0.5, 0.5, 0.5*/
      ...
      while (random.face[i] > 6 || random.face[i] < 1)
        random.face[i] = int(1 + rand() / float(RAND_MAX) * 5. + 0.5);
      if (myid == 0)
        cout << " face of the dice " << random.face[i] << endl;
      random.sgnX[i] = 2;
      srand(p.seedsign + i / numOfLensPerSnap * 8);
      while (random.sgnX[i] > 1 || random.sgnX[i] < 0)
        random.sgnX[i] = int(rand() / float(RAND_MAX) + 0.5);
      random.sgnY[i] = 2;
      while (random.sgnY[i] > 1 || random.sgnY[i] < 0)
        random.sgnY[i] = int(rand() / float(RAND_MAX) + 0.5);
      random.sgnZ[i] = 2;
      while (random.sgnZ[i] > 1 || random.sgnZ[i] < 0)
        random.sgnZ[i] = int(rand() / float(RAND_MAX) + 0.5);
      if (random.sgnX[i] == 0)
        random.sgnX[i] = -1;
      if (random.sgnY[i] == 0)
        random.sgnY[i] = -1;
...
void computeReplications(double fov, double boxl, double Ds, int myid, double &fovradiants, int &nrepperp)
{
  ...
  fovradiants = fov / 180. * M_PI;
  if (Ds * tan(fovradiants / 2.0) <= boxl / 2.0)
    nrepperp = 0;
  else
    nrepperp = ceil((Ds * tan(fovradiants / 2) - boxl / 2.0) / boxl);
}
          ...
          for (int ni = -lens.nrepperp[isnap]; ni <= lens.nrepperp[isnap]; ni++)
            for (int nj = -lens.nrepperp[isnap]; nj <= lens.nrepperp[isnap]; nj++)
            {
              ...
              if (fabs(rai) <= fovradiants * (1. + 2. / p.npix) * 0.5 && fabs(deci) <= fovradiants * (1. + 2. / p.npix) * 0.5)
              {
                ...
                {
                  if (rand() / float(RAND_MAX) < 1. / pow(2, p.snopt))
                    ms.push_back(pow(2, p.snopt) * num_float1);
                  else
                    ms.push_back(0.);
                }
              }
...
int createDensityMaps(InputParams &p, Lens &lens, Random &random, int isnap,
                      unsigned int ffmin, unsigned int ffmax, string File, double fovradiants, double rcase,
                      ...
                      valarray<float> (&mapxytoti)[6], int (&ntotxyi)[6], int myid)
{
  ...
  for (unsigned int ff = ffmin; ff < ffmax; ff++)
  {
    ...
    gadget = new Gadget;
    gadget->xx0.resize(data.npart[0]);
    gadget->yy0.resize(data.npart[0]);
    gadget->zz0.resize(data.npart[0]);
    gadget->xx1.resize(data.npart[1]);
    gadget->yy1.resize(data.npart[1]);
    gadget->zz1.resize(data.npart[1]);
    gadget->xx2.resize(data.npart[2]);
    gadget->yy2.resize(data.npart[2]);
    gadget->zz2.resize(data.npart[2]);
    gadget->xx3.resize(data.npart[3]);
    gadget->yy3.resize(data.npart[3]);
    gadget->zz3.resize(data.npart[3]);
    gadget->xx4.resize(data.npart[4]);
    gadget->yy4.resize(data.npart[4]);
    gadget->zz4.resize(data.npart[4]);
    gadget->xx5.resize(data.npart[5]);
    gadget->yy5.resize(data.npart[5]);
    gadget->zz5.resize(data.npart[5]);
    xx[0][0] = &gadget->xx0[0];
    xx[0][1] = &gadget->yy0[0];
    xx[0][2] = &gadget->zz0[0];
    xx[1][0] = &gadget->xx1[0];
