...
double TSC_WC6_conv (double *r, size_t dim, void *p){
  ...
  double rrj=sqrt( pow(r[0]+ri[0]-rj[0],2)+pow(r[1]+ri[1]-rj[1],2) ); // Distance between the particle and the integrand position
  return (1./pow(h[0],3))*WC6_projected_kernel(rrj/h[0])*weight(r[0],0,h[1])*weight(r[1],0,h[1]);
...
valarray<float> gridist_w (vector<float> x, vector<float> y , vector<float> w, vector<float> h, int nn){
  ...
  gsl_rng *rand;
  struct params p;
  p.h_TSC=1.; // TSC kernel smoothing length (TSC kernel radius is 1.5xh_TSC)
  gsl_monte_function  W = {&TSC_WC6_conv, 2, &p};
  size_t calls = pow(10,2);
  gsl_rng_env_setup ();
  T = gsl_rng_default;
  ...
  for (int i=0; i<n0; i++){
    ...
    for (int ix=grx-gr; ix<=grx+gr; ix++){ // All the possible bins in x-axis englobed by r (SPH kernel)

      for (int jy=gry-gr; jy<=gry+gr; jy++){ // All the possible bins in y-axis englobed by r (SPH kernel)
      
        for(int ii=-1;ii<=1;ii++)for(int jj=-1;jj<=1;jj++){

           if(ix+ii<nn && jy+jj<nn && ix+ii>=0 && jy+jj>=0 && w[i]<=1){ //wi<1 to test!!!! To remind to remove!!!!

            p.xi=ix+ii+.5; p.yi=jy+jj+.5;
            gsl_monte_plain_integrate (&W, xl, xu, 2, calls, rand, s,&res, &err);
            grxy[ix+ii+nn*(jy+jj)] += w[i]*res;
...
//cout << filredshiftlist;
  ...
  for(int nsnap=0;nsnap<nsnaps;nsnap++){
	...
	double mnorm=accumulate(ms.begin(), ms.end(), 0.);
        double m_too_high=0;
        for(int i=0;i<ms.size();i++)if(ms[i]>1) m_too_high+=ms[i]; //Subtracting masses above 10^10xM_sun/h
